...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<?> taxonomy(TaxonomyChangeException ex, HttpServletRequest req) {
    ...
    public ResponseEntity<?> stale(CasConflictException ex, HttpServletRequest req) {
        return envelope(HttpStatus.CONFLICT, "STALE_VERSION", ex.getMessage(), req);
    }

    @ExceptionHandler(IdentityCollisionException.class)
    public ResponseEntity<?> identity(IdentityCollisionException ex, HttpServletRequest req) {
    ...
    public ResponseEntity<?> attributes(AttributeViolationException ex, HttpServletRequest req) {
    ...
    public ResponseEntity<?> evidence(EvidenceGateException ex, HttpServletRequest req) {
    ...
    public ResponseEntity<?> bundle(BundleComponentException ex, HttpServletRequest req) {
    ...
    public ResponseEntity<?> immutableField(com.tazzzo.catalog.tx.ImmutableFieldException ex,
    ...
    public ResponseEntity<?> variantPack(com.tazzzo.catalog.tx.VariantPackException ex,
                                                 HttpServletRequest req) {
    ...
    public ResponseEntity<?> evidenceImmutable(RuntimeException ex, HttpServletRequest req) {
    ...
    public ResponseEntity<?> evidenceContract(
    ...
    public ResponseEntity<?> badTimestamp(Exception ex, HttpServletRequest req) {
    ...
    public ResponseEntity<?> methodNotAllowed(Exception ex, HttpServletRequest req) {
        return envelope(HttpStatus.METHOD_NOT_ALLOWED, "METHOD_NOT_ALLOWED", ex.getMessage(), req);
    }

    @ExceptionHandler(org.springframework.web.HttpMediaTypeNotSupportedException.class)
    public ResponseEntity<?> mediaType(Exception ex, HttpServletRequest req) {
        return envelope(HttpStatus.UNSUPPORTED_MEDIA_TYPE, "UNSUPPORTED_MEDIA_TYPE", ex.getMessage(), req);
    }

    @ExceptionHandler(org.springframework.web.servlet.resource.NoResourceFoundException.class)
    public ResponseEntity<?> noRoute(Exception ex, HttpServletRequest req) {
    ...
    public ResponseEntity<?> mongo(MongoWriteException ex, HttpServletRequest req) {
    ...
    /**
     * The single seam through which every error leaves this advice — and therefore the single place
     * where the SURFACE decides the SHAPE (ERR1-FRAMEWORK-1).
     *
     * <p>The whole {@code /catalog/v1/**} prefix is classified public before Spring knows whether a
     * controller exists, so an unmapped public URL, an unsupported method, or a binding failure
     * never reaches a consumer controller and would otherwise fall through to the nested CMS
    ...
    /**
     * ERR-1 for framework failures on the public surface. Codes collapse to the consumer
     * vocabulary and messages are generic (Q5-d, L-5): a shopper — or anyone probing — learns
     * nothing about routes, methods, bindings or internals from the body.
     */
    private ResponseEntity<?> publicEnvelope(HttpStatus status, String internalCode, String requestId) {
        String code;
        String message;
        HttpStatus publicStatus;
        if (status == HttpStatus.NOT_FOUND) {
            publicStatus = HttpStatus.NOT_FOUND;
            code = "NOT_FOUND";
            message = "not found";
        } else if (status.is5xxServerError()) {
            publicStatus = HttpStatus.SERVICE_UNAVAILABLE;
            code = "SERVICE_UNAVAILABLE";
            message = "service unavailable";
        } else {
            // 405, 415, 400, 409, 422 — all "the request itself was not acceptable"
            publicStatus = HttpStatus.BAD_REQUEST;
