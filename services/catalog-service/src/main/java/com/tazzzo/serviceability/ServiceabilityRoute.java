 ...
 * number wins; priorities are unique within an area (across ALL routes, active or not — an
 * inactive route still reserves its value so re-activation can never create a tie), so no
 * tie-break is ever needed and no resolution can depend on Mongo natural order.
 * {@code priority} is DELIBERATELY unbounded above (any non-negative int): with ≤20 routes per
 * area and uniqueness enforced, a large value is merely a low-preference candidate — harmless.
 */
